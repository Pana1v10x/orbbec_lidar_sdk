...
using DataPacketHeader =
    std::variant<MultiLineDataPacketHeader, SingleLineDataPacketHeader>;
...
class CommandResponse {
   public:
    ...
    void getData(void *data, size_t size, size_t *read_size) {
        const size_t min_size = std::min<size_t>(size, data_.size());
        *read_size = min_size;
...
class DataResponse {
   public:
    explicit DataResponse(const LidarType type) : type_(type) {}

    void SingleLineHeaderToHeader() {
        auto &header = std::get<SingleLineDataPacketHeader>(header_);
    ...
    void MultiLineHeaderToHeader() {
        auto &header = std::get<MultiLineDataPacketHeader>(header_);
        header.packet_header1 = ntohl(header.packet_header1);
        header.packet_header2 = ntohs(header.packet_header2);
    ...
    Status setData(const uint8_t *data, size_t size) {
        if (type_ == LidarType::SINGLE_LINE) {
            if (size < sizeof(SingleLineDataPacketHeader)) {
                return Status::INVALID;
            }
            SingleLineDataPacketHeader single_line_header{};
            memcpy(&single_line_header, data,
                   sizeof(SingleLineDataPacketHeader));
            header_ = single_line_header;
            // Important: convert to host order
            SingleLineHeaderToHeader();
            ...
            memcpy(data_.data(), data + sizeof(SingleLineDataPacketHeader),
                   data_.size());
            memcpy(&end,
                   data + sizeof(SingleLineDataPacketHeader) + data_.size(),
                   sizeof(uint32_t));
        } else if (type_ == LidarType::MULTI_LINE) {
            if (size < sizeof(MultiLineDataPacketHeader)) {
                return Status::INVALID;
            }
            MultiLineDataPacketHeader multi_line_header{};
            memcpy(&multi_line_header, data, sizeof(MultiLineDataPacketHeader));
            header_ = multi_line_header;
            // Important: convert to host order
            MultiLineHeaderToHeader();

            timestamp_ = std::get<MultiLineDataPacketHeader>(header_).timestamp;
            size_t data_length =
                std::get<MultiLineDataPacketHeader>(header_).data_length;
            data_.resize(data_length);
            memcpy(data_.data(), data + sizeof(MultiLineDataPacketHeader),
                   data_.size());
            memcpy(&end,
                   data + sizeof(MultiLineDataPacketHeader) + data_.size(),
                   sizeof(uint32_t));
        }
        return Status::OK;
    }
