...
TcpServer::TcpServer(std::shared_ptr<uvw::loop> loop, const std::string &ip,
    ...
    server_ = loop_->resource<uvw::tcp_handle>();
    server_name_ = "TCP Server " + ip + ":" + std::to_string(port);

    server_->on<uvw::error_event>(
        [](const uvw::error_event &, uvw::tcp_handle &) {
            std::cerr << "Server Error!" << std::endl;
        });

    server_->on<uvw::listen_event>(
        [this](const uvw::listen_event &, uvw::tcp_handle &handle) {
            // Accept a new connection, get peer address and port
            client_handle_ = handle.parent().resource<uvw::tcp_handle>();
            client_handle_->on<uvw::error_event>(
                [](const uvw::error_event &, uvw::tcp_handle &) {
                    std::cerr << "Client Error!" << std::endl;
                });

            client_handle_->on<uvw::data_event>(
                [this](const uvw::data_event &event,
                       uvw::tcp_handle &client_handle) {
                    std::vector<uint8_t> received_data(
                        event.data.get(), event.data.get() + event.length);
                    onDataReceived(received_data, client_handle);
                });

            client_handle_->on<uvw::end_event>(
                [](const uvw::end_event &, uvw::tcp_handle &client_handle) {
                    client_handle.close();
                });

            handle.accept(*client_handle_);
            client_handle_->read();
    ...
    std::cout << "Server bound to " << ip << ":" << port << std::endl;
    server_->listen();
...
void TcpServer::onDataReceived(const std::vector<uint8_t> &data,
    ...
    if (data.size() < sizeof(DataHeader) + payload_length) {
        ...
        std::cerr << "Expected: payload size " << payload_length << std::endl;
    ...
    std::cout << server_name_ << " Received command: " << command_id
              << std::endl;
    if (static_cast<ob::LidarCommandInterface>(command_id) ==
        ob::LidarCommandInterface::ENABLE_STREAMING) {
        uint32_t enable = 0;
        std::memcpy(&enable, payload.data(), sizeof(uint32_t));
        if (enable && send_thread_ == nullptr) {
            send_data_ = true;
            send_thread_ =
                std::make_shared<std::thread>(&TcpServer::sendFakeData, this);
...
std::vector<uint8_t> TcpServer::buildResponseData(const uint16_t &command_id) {
    ...
    if (const uint16_t high_byte = (command_id >> 8) & 0xFF;
        ...
        // set command;
        switch (auto command = static_cast<LidarCommandInterface>(command_id)) {
            ...
            case LidarCommandInterface::GET_TX_VOLTAGE:
                ...
                return payload;
            default:
                std::cerr << "unhandled command " << command_id << std::endl;
                return payload;
        }
    } else {
...
void TcpServer::onResponseData(const uint16_t &command_id,
                               uvw::tcp_handle &handle) {
...
void TcpServer::sendFakeData() {
    ...
    fake_point.tag = 5;
    fake_points.reserve(10);
    for (int i = 0; i < 10; i++) {
        fake_points.emplace_back(fake_point);
    }
    const uint16_t frame_length = sizeof(ob::LidarPoint) * fake_points.size();
    ...
    std::get<ob::MultiLineDataPacketHeader>(header).frame_index = htons(0);
    std::get<ob::MultiLineDataPacketHeader>(header).data_type = 0x02;
    std::get<ob::MultiLineDataPacketHeader>(header).packet_header1 =
        htons(DATA_TYPE_RESPONSE_HEADER);
    std::get<ob::MultiLineDataPacketHeader>(header).packet_header2 = 0xEB90;
    std::get<ob::MultiLineDataPacketHeader>(header).data_length =
        htons(frame_length);
    ...
    while (send_data_) {
        ...
        client_handle_->write(const_cast<char *>(data),
