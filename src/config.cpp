...
// helper functions
LidarType parseLidarType(const std::string& type) {
    // convert to lower case
    std::string lower_type = type;
    std::transform(lower_type.begin(), lower_type.end(), lower_type.begin(),
                   ::tolower);
    if (lower_type == "single_line") {
        return LidarType::SINGLE_LINE;
    } else if (lower_type == "multi_line") {
        return LidarType::SINGLE_LINE;
    }
...
LogLevel parseLogLevel(const std::string& level) {
    ...
    if (level == "error" || level == "ERROR") return LogLevel::ERR;
...
NetworkConfig::NetworkConfig(LidarType lidar_type,
...
bool LoggerConfig::shouldLogToCallback() const { return log_to_callback_; }
            ...
            for (const auto& [_, device_config_node] : *device_table) {
                ...
                try {
                    ...
                    if (json.contains("type")) {
                        params_.lidar_type =
                            parseLidarType(json["type"].get<std::string>());
                    }
                    if (json.contains("enable_heartbeat")) {
                        params_.enable_heartbeat =
                            json["enable_heartbeat"].get<bool>();
                    }
                    if (json.contains("max_heartbeat_fail_allowed")) {
                        params_.max_heartbeat_fail_allowed =
                            json["max_heartbeat_fail_allowed"].get<int>();
                    }
                    if (json.contains("heartbeat_interval_ms")) {
                        params_.heartbeat_interval_ms =
                            json["heartbeat_interval_ms"].get<long>();
                    }
                    if (json.contains("option_timeout_ms")) {
                        params_.option_timeout_ms =
                            json["option_timeout_ms"].get<long>();
                    }
                    if (json.contains("network")) {
                        auto network_config = json["network"];
                        network_config_ = std::make_shared<NetworkConfig>();
                        network_config_->loadFromJsonString(
                            network_config.dump(2));
                        params_.protocol_type =
                            network_config_->getProtocolType();
                    }
...
DeviceConfig::DeviceConfig(std::string device_name, LidarType lidar_type,
                           LidarProtocolType protocol_type)
    : device_name_(std::move(device_name)) {
    params_.lidar_type = lidar_type;
}
...
void DeviceConfig::setOptionTimeout(std::chrono::milliseconds timeout) {
    params_.option_timeout_ms = timeout.count();
}

std::chrono::milliseconds DeviceConfig::getOptionTimeout() const {
    return std::chrono::milliseconds(params_.option_timeout_ms);
}

LidarType DeviceConfig::getLidarType() const { return params_.lidar_type; }

LidarProtocolType DeviceConfig::getProtocolType() const {
    CHECK_NOTNULL(network_config_);
    return network_config_->getProtocolType();
}

const DeviceParams& DeviceConfig::getParams() const { return params_; }

void DeviceConfig::setParams(const DeviceParams& params) { params_ = params; }
...
DeviceConfigBuilder& DeviceConfigBuilder::setType(const std::string& type) {
    params_.lidar_type = parseLidarType(type);
    return *this;
}

DeviceConfigBuilder& DeviceConfigBuilder::setProtocolType(
    const std::string& protocol_type) {
    params_.protocol_type = parseProtocolType(protocol_type);

    return *this;
}

DeviceConfigBuilder& DeviceConfigBuilder::setIP(const std::string& ip) {
    params_.ip = ip;
    return *this;
}

DeviceConfigBuilder& DeviceConfigBuilder::setPort(uint16_t port) {
    params_.port = port;
    return *this;
}
...
DeviceConfigBuilder& DeviceConfigBuilder::setNetworkConfigJsonString(
    ...
    params_.protocol_type = network_config_->getProtocolType();
    return *this;
}
...
DeviceConfigBuilder& DeviceConfigBuilder::setEnableHeartbeat(
    bool enable_heartbeat) {
    params_.enable_heartbeat = enable_heartbeat;
    return *this;
}

std::shared_ptr<DeviceConfig> DeviceConfigBuilder::build() {
    // check if all required fields are set
    if (device_name_.empty() || params_.lidar_type == LidarType::UNKNOWN) {
    ...
    if (network_config_ == nullptr &&
        params_.protocol_type != LidarProtocolType::UNKNOWN &&
        !params_.ip.empty() && params_.port != 0) {
        network_config_ =
            getDefaultNetworkConfig(params_.lidar_type, params_.protocol_type,
                                    params_.ip, params_.port);
    ...
    auto device_config = std::make_shared<DeviceConfig>(
        device_name_, params_.lidar_type, params_.protocol_type);

    if (network_config_ == nullptr) {
        std::cerr << "Network config is not set, using default network config "
                     "for model: "
                  << static_cast<int>(params_.lidar_type) << ", protocol type: "
                  << static_cast<int>(params_.protocol_type) << std::endl;
        network_config_ =
            getDefaultNetworkConfig(params_.lidar_type, params_.protocol_type,
                                    params_.ip, params_.port);
    }
    ...
    device_config->setParams(params_);
...
bool DeviceConfigBuilder::checkDeviceConfig() const {
    ...
    if (network_config_->getProtocolType() != params_.protocol_type) {
...
std::shared_ptr<NetworkConfig> DeviceConfigBuilder::getDefaultNetworkConfig(
    ...
    uint16_t port) {
    auto network_config =
        std::make_shared<NetworkConfig>(type, protocol_type, ip, port);
